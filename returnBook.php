...
<div id="div4">
  <form action="returnBook.php" method="POST">
    <table id="table_body">
    <tr>
      <td>Book ID of the book you want to return:</td>
      <td><input type="text" name="bookDetails" required></td>
    </tr>
    <tr>
...
<?php
  ...
  if(isset($_POST['bookDetails']))
  {
    ...
    $sql_to_check="SELECT * FROM issue WHERE f_name='$name' AND b_id='$issue_book'";
    $sql_to_check_query=mysql_query($sql_to_check,$conn);
    if(mysql_num_rows($sql_to_check_query))
    {
      
    $checkNumber="SELECT * from bookDetails WHERE book_id='$issue_book'";
    $checkNumber_query=mysql_query($checkNumber,$conn);
    $number=mysql_fetch_array($checkNumber_query);
      ...
      $insert_query="INSERT INTO return_book VALUES ('$bookInsertId','$bookInsertAuthor','$dateIssue','$dateReturn','$userInsertFname','$userInsertLname')";
      ...
      $return_book="DELETE FROM issue WHERE b_id='$issue_book'";
      $return_book_query=mysql_query($return_book,$conn);
  ?>
...
<script>
  ...
  setTimeout(function()
  {
    window.open('returnBook.php','_self');
  },delay);
</script>
<?php
}
...
?>
 <div class="alert">
  <span class="closebtn"></span> 
  <strong>Wait!</strong>No such book available!!!!
</div>
